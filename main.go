...
var (
	domain         = "www.centriassistenza.org"
...
func scrape() {
	...
	c.OnRequest(func(r *colly.Request) {
		...
		siteVisited = append(siteVisited, r.URL.String())
	})

	for _, sito := range sitesToScrape {
		c.Visit(sito)
	}

	c.Wait()

	//Check if some site from the list was skipped
	var sitiNonVisitati []string

	for i := 0; i < len(sitesToScrape); i++ {
		visitato := false
		for j := 0; j < len(siteVisited); j++ {
			if sitesToScrape[i] == siteVisited[j] {
				visitato = true
				break
			}
		}
		...
		if !visitato {
			sitiNonVisitati = append(sitiNonVisitati, sitesToScrape[i])
		}
	}

	fmt.Print(sitiNonVisitati)
	saveToJson(lsScan, filenameOutput)
}
